...
impl LocalModel {
    ...
    ///
    /// Note: This is different from `detach_from_endpoint` which only unregisters
    /// the model card. This function unregisters the underlying endpoint instance
    /// so the worker is completely removed from the routing pool.
